...
const receiveFilterByName = (filterResults, name) => ({
...
function filtersByNumericValues(filterResults, name, column, comparison, value) {
  const newFilters = filterResults.filter((planet) => planet.name.match(name.toLowerCase()));
...
export const filterByName = (results, name, filters) => {
  ...
  return (dispatch) => (
    dispatch(receiveFilterByName(newFilters, name))
  );
};
